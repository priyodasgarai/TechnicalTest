...
<div class="box">
    <div class="box-header">
        ...
        <a  href="{{url('/admin/add-edit-category')}}" class="btn btn-mini btn-primary pull-right button_class">{{trans('labels.31')}}</a>
    </div>
    <!-- /.box-header -->           
    @if(!empty($categories))
    <div class="box-body">
            ...
            <tbody>
                ...
                <tr>               
                    ...
                    <td>
                        ...
                        {{'Root'}}
                        @else
                        {{ucwords($data->parentcategory->category_name)}}
                        @endif
                    </td>
                    ...
                    <td>
                        <input type="hidden" id="category_details_{{$data->id}}" value="{{json_encode($data)}}">
                        @if ($data->status==1)  
                        <a class="btn btn-mini primary" onclick="change_category_status('{{base64_encode(env('APP_KEY').'||'.$data->id)}}')" >
...
<script>
                                                        ...
                                                        var dec = window.atob(id);
                                                                var res = dec.split('||');
                                                                var data_id = res[1];
                                                                var data_details = $("#category_details_" + data_id).val();
